...
const {
  ProcessManager, getDate, timeout, readFile,
...
class AEManager {
  ...
  /**
   * Is After Effect running and ready? Check startup file.
   *
   * @returns {Boolean}
   * @author Eignart
   * @since 2019.03.24
  ...
  async isReady() {
    ...
    if (aeProcFile && aeProcess.isExist() && aeProcess.PID === aeProcFile.PID) {
      console.log(`[${getDate()}] (AEManager) AfterFX allready running`);
      return true;
    }
...
module.exports = {
  ProcessManager, getDate, timeout, AEManager, readFile,
};
