...
class AEManager {
   ...
   */
  async isReady() {
    await this.isAeRunning();

    const process = this.pm.getProcessInfo();
    const processFile = readFile(this.aeRunningCheckFile);
    let aeProcess = await process;
    const aeProcessFile = await processFile;
    ...
    if ((aeProcFile && !aeProcess.isExist())
      ...
      console.log(`[${getDate()}] (AEManager) Remove startup check file.`);
      this.removeAeRunningCheckFile();
    }

    if (!aeProcess.isExist()) {
      console.log(`[${getDate()}] AE process not exist. Remove 'isRunning' file and start AE process.`);
      this.removeAeRunningCheckFile();
  ...
  /**
   * Check process status. Wait until process status is Running.
   *
   * @param {void}
   * @returns {Promise}
   * @author Eignart
  ...
  async isAeRunning() {
    return new Promise(async (res) => {
      ...
      const i = setInterval(async () => {
        ...
        // remove startup file if previous process killed.
        if (app.pm.processKilled && !fileRemoved) {
          this.removeAeRunningCheckFile();
          fileRemoved = true;
        }

        if (aeProcess && aeProcess.Status !== 'Not Responding') {
          clearInterval(i);
          res();
        }
      }, 1000);
